...
void writeArrayToFilePNG(const char* filePath, const unsigned char* data, int width, int height, int format)
{
	png_byte color_type = PNG_COLOR_TYPE_RGBA;
	png_byte bit_depth = 8;
	...
	color_type = (format == 3 ? PNG_COLOR_TYPE_RGB : PNG_COLOR_TYPE_RGBA);
	row_pointers = (png_bytep*)malloc(sizeof(unsigned char*) * height);
	//row_pointers[0] = (png_bytep)malloc(sizeof(unsigned char) * (format == 3 ? 3 : 4) * width * height);
	row_pointers[height - 1] = (png_bytep)data;
	for (int y = 0; y < height; y++)
		row_pointers[height - 1 - y] = row_pointers[height - 1] + (y * (format == 3 ? 3 : 4) * width);
